    ...
    public class AgencySuperagents : ControllerBase
    {
        ...
        public async Task<JObject> Get()
        {
            ...
            try
            {
                ...
                var response = dBHandler.AgencySuperAgents();
                ...
                else
                {
                    ...
                    response_json.Add("RESPONSEMESSAGE", "Failed to get superagents!");
                }
            }
            catch (Exception ex)
            {
        ...
        // GET api/<AgencySuperagents>/5
        [HttpGet("GetSuperagent/{id}")]
        public async Task<JObject> Get(int id)
        {
            ...
            try
            {
                var response = dBHandler.AgencySuperAgent(id);
                if (response.Rows.Count > 0)
                {
                    JObject child = new JObject();
                    foreach (DataColumn col in response.Columns)
                ...
                {
                    response_json.Add("RESPONSECODE", "01");
                    response_json.Add("RESPONSEMESSAGE", "Failed to get superagent!");
                }
            }
            catch (Exception ex)
            {
