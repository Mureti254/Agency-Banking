    ...
    public class FileLogHandler
    {
        ...
        public static void log_message_fields(string module, string messagetype, string message)
        {
            ...
            try
            {
                ...
                //f.WriteLine(message);
                //f.Close();
                string logfilespath = dbhandler.GetRecords("parameters", "LOG_FILES_PATH").Rows[0]["item_value"].ToString() + DateTime.Now.ToString("yyyy_MM_dd");
                ...
                if (messagetype == "ERROR")
                {
                    ...
                    strFileName = logfilespath + "\\" + module + filename + "_Errorlog.MSG";//Program.configuration.GetSection("Data").GetSection("Errorlogger").Value ?? string.Empty;
                }
                else
                {
                    strFileName = logfilespath + "\\" + module + filename + ".MSG"; //Program.configuration.GetSection("Data").GetSection("Errorlogger").Value ?? string.Empty;
                }
